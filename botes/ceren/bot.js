...
const sqlitePath = config_data.sqlite_path
  ...
  : path.join(__dirname, "ceren.sqlite");
...
const normalizeLang = (value) => {
  ...
  return supportedLangs.has(v) ? v : "tr";
};
...
//             let main_menu_buttons = Markup.inlineKeyboard([[Markup.button.url("🔗  Davet Et Ücretsiz Al  🔗", `https://t.me/share/url?url=${encodeURIComponent(`https://t.me/${config_data.bot_username}?start=${ctx.from.id}`)}`)], [Markup.button.callback(`👑  VIP KANAL  👑`, "wip_channel")], [Markup.button.url("💋  Özel Söhbet  💋", config_data.private_chat)], [Markup.button.callback("🍑  İçerikler  🍑", "content"), Markup.button.callback("🥵  Show  🥵", "show")], [Markup.button.callback("💸  Cüzdan  💸", "wallet")]])
